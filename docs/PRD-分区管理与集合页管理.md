...
3. **分区列表表格**：展示序号、分区名称、**分区类型**、状态、操作人、操作时间、操作
   - **分区类型**（前台版式契约）：
     ...
     - **`card-grid`**：**卡片网格** — 宫格/卡片式多列铺排（也常称「卡片」布局）。
     - **一级无二级 Tab**：字段 `layoutType` 落在一级分区；固定行「全部」为只读 **Feeds 流**；其它行表格内 **Select** 可即时改一级类型。
     - **一级含二级 Tab**（`subTabs` 非空）：**一级不再使用 `layoutType`**；列表格展示 **「二级 N 项 · 点击配置」**，Popover 内为每个二级 Tab 单独选择分区类型。
   - **新建 / 编辑分区**（弹窗）：分区名称；**启用二级 Tab** 开关 — 开启后隐藏一级分区类型，改为 **二级 Tab 列表**（每行：名称 + 分区类型，可增删）；关闭开关则仅配置一级 `layoutType`。
   - 操作列含 **「编辑分区」**（打开上述弹窗）与 **「管理内容」**（进模块配置页）。
   - 规则：未配置时前台可按 `feeds` 默认处理。

4. **列表标题栏**：列表区块标题与数量
   - 文案：标题「列表」；数量标签「N 条」

5. **新建/编辑分区弹窗**：见上「二级 Tab」规则；提交时要么写一级 `layoutType`（且无 `subTabs`），要么写 `subTabs` 并清空一级 `layoutType`。
