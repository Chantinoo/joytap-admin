...
3. **分区列表表格**：展示序号、分区名称、**分区类型**、状态、操作人、操作时间、操作
   - **分区类型**（字段 `layoutType`，前台版式契约）：
     - **`feeds`**：**Feeds 流** — 纵向列表/信息流，典型为左缩略图 + 标题与元信息（时间线式条目）。
     - **`card-grid`**：**卡片网格** — 宫格/卡片式多列铺排（也常称「卡片」布局；比单列 Feed 更强调封面墙、专题入口）。
     - 固定行「全部」展示类型为 **Feeds 流**（只读标签，不可改）；其它行表格内 **Select** 可即时修改并提示成功。
   - 交互：内联改名称、拖拽排序、状态开关、删除、管理内容等（与当前实现一致）
   - 文案：列头含「分区类型」；下拉项展示「Feeds 流」「卡片网格」
   - 规则：未配置时前台可按 `feeds` 默认处理；新建分区弹窗必选类型，默认 **Feeds 流**

4. **列表标题栏**：列表区块标题与数量
   - 文案：标题「列表」；数量标签「N 条」

5. **新建/编辑分区弹窗**：分区名称（必填）+ **分区类型**（必选，默认 Feeds 流）；提交创建或更新时写入 `layoutType`。
...
1. **面包屑**：当前路径，前置可点
   ...
   - 文案：例如「论坛管理 / 分区管理 / 攻略」；页头主标题旁展示 **分区类型** 标签（非固定分区），与列表配置一致
