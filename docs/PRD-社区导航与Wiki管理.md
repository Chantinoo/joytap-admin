...
**典型场景**：运营希望在前台顶部展示「讨论」「数据库（下拉：宠物、箱子、箭矢制作等）」「卡片」「地图」「充值优惠」。需要在 Wiki 管理中先创建这些导航并配置字段，再在社区导航中按上述结构编排。
...
|----|------|
| 拖拽 | 拖拽手柄（讨论无），用于调整顺序 |
| 导航名称 | 显示名称；讨论显示「默认」标签；下拉菜单显示「N 个子项」 |
| 链接 | 讨论：灰色标签样式；Wiki/充值：蓝色可点击样式（仅展示，不跳转）；下拉菜单：竖向列出所有子项链接 |
| 状态 | 启用/禁用开关（讨论无开关） |
| 操作 | 编辑、删除（讨论无） |
...
- 链接自动生成：Wiki 导航为 `/wiki/[key]`，讨论为 `/forum/list`，充值入口为用户配置的 link
...
- 讨论固定 `id: 'discussion'`，链接固定 `/forum/list`
