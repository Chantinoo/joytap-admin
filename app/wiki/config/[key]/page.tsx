...
import {
  ...
  Tooltip, Popconfirm, message, Checkbox,
} from 'antd'
...
export default function WikiConfigPage() {
  const router = useRouter()
  const params = useParams()
  const wikiKey = params.key as string
  ...
  const handleFieldSave = () => {
    fieldForm.validateFields().then(values => {
      ...
      if (editingField) {
        ...
        setFields(prev => [...prev, { ...defaults, ...values, i18n: { zh: values.label }, order: maxOrder + 1 }])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10, marginBottom: 20 }}>
              {LIST_STYLES.map(s => (
                <div key={s.id} onClick={() => {
                  ...
                  if (s.requireImage) {
                    const firstImg = sortedFields.find(f => f.visible && f.type === 'image')
              ...
              <div style={{ width: 260, flexShrink: 0, border: '1px solid #E5E7EB', borderRadius: 8, background: '#FAFAFA', padding: '12px 14px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                  {sortedFields.filter(f => f.visible).map(f => {
                    ...
                    const checkedImageKeys = validListFieldKeys.filter(k => { const ff = sortedFields.find(x => x.key === k); return ff?.type === 'image' })
                    const checkedTextKeys = validListFieldKeys.filter(k => { const ff = sortedFields.find(x => x.key === k); return ff && ff.type !== 'image' })
      ...
      {/* 新增 / 编辑字段弹窗 */}
      <Modal title={editingField ? '编辑字段' : '新增字段'} open={fieldModalOpen} onOk={handleFieldSave} onCancel={() => setFieldModalOpen(false)} okText="保存" cancelText="取消" width={480}>
        <Form form={fieldForm} layout="vertical" style={{ marginTop: 16 }}>
          ...
          <Form.Item name="type" label="字段类型" rules={[{ required: true }]}>
            <Select options={[{ label: 'text（文本）', value: 'text' }, { label: 'number（数字）', value: 'number' }, { label: 'image（图片）', value: 'image' }, { label: 'tag（标签）', value: 'tag' }, { label: 'rich-table（富媒体表格）', value: 'rich-table' }]} />
          </Form.Item>
          <Form.Item name="required" label="是否必填" valuePropName="checked"><Switch /></Form.Item>
        </Form>
      </Modal>
