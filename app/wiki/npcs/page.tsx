...
interface NPC {
  ...
  function: string; type: string; visible: boolean
  [key: string]: unknown
}
...
const initialFields: WikiField[] = [
  ...
  { key: 'location', label: '位置',   i18n: { zh: '位置',   en: 'Location' }, type: 'text',   visible: true,  listDisplay: true,  sortable: false, filterable: true,  required: false, order: 5 },
  { key: 'function', label: '功能',   i18n: { zh: '功能',   en: 'Function' }, type: 'text',   visible: true,  listDisplay: true,  sortable: false, filterable: false, required: false, order: 6 },
  { key: 'dialogue', label: '对话',   i18n: { zh: '对话',   en: 'Dialogue' }, type: 'text',   visible: false, listDisplay: false, sortable: false, filterable: false, required: false, order: 7 },
]

const mockNPCs: NPC[] = [
  { id: 1001, name: '卡普拉',     icon: '👩', location: '普隆德拉',     function: '仓库管理',   type: '服务', visible: true },
  { id: 1002, name: '铁匠',       icon: '🔨', location: '普隆德拉',     function: '装备强化',   type: '服务', visible: true },
  { id: 1003, name: '商人',       icon: '🛒', location: '普隆德拉',     function: '道具买卖',   type: '商店', visible: true },
  { id: 1004, name: '转职官',     icon: '📜', location: '普隆德拉骑士团', function: '职业转职', type: '任务', visible: true },
  { id: 1005, name: '冒险家协会', icon: '🏛️', location: '普隆德拉',     function: '任务发布',   type: '任务', visible: true },
  { id: 1006, name: '宠物商人',   icon: '🐾', location: '普隆德拉',     function: '宠物相关',   type: '商店', visible: true },
  { id: 1007, name: '传送师',     icon: '🌀', location: '各主城',       function: '城市传送',   type: '服务', visible: true },
  { id: 1008, name: '技能导师',   icon: '📖', location: '各主城',       function: '技能重置',   type: '服务', visible: false },
]
...
export default function WikiNPCsPage() {
  ...
  const filteredNPCs = mockNPCs.filter(npc => {
    const matchSearch = npc.name.includes(searchText) || String(npc.id).includes(searchText)
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ background: '#fff', borderRadius: 8, border: '1px solid #E5E7EB', overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid #E5E7EB', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <h2 style={{ margin: 0, fontSize: 16, fontWeight: 600, color: '#111827' }}>NPC 管理</h2>
            <p style={{ margin: '4px 0 0', fontSize: 13, color: '#6B7280' }}>管理前台 Wiki NPC 页面的数据与字段配置</p>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            {activeTab === 'data'
              ? <Button type="primary" icon={<Plus size={14} />} style={{ borderRadius: 6 }}>新增 NPC</Button>
        ...
        {activeTab === 'data' && (
          <div style={{ padding: '16px 20px' }}>
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              <Input placeholder="搜索 NPC 名称或 ID" prefix={<Search size={14} style={{ color: '#9CA3AF' }} />}
              ...
              <div style={{ marginLeft: 'auto', fontSize: 13, color: '#6B7280', display: 'flex', alignItems: 'center' }}>共 {filteredNPCs.length} 条</div>
            </div>
            <Table dataSource={filteredNPCs} columns={dataColumns} rowKey="id" size="small"
      ...
      >
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          >
            <Input placeholder="如：npc_level" disabled={!!editingField} />
          </Form.Item>
          <Form.Item name="label" label="显示名称（中文）" rules={[{ required: true, message: '请输入显示名称' }]}>
            <Input placeholder="如：NPC 等级" />
          </Form.Item>
